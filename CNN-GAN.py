...
# def trainNetwork(dataloader,model,lossF,optim):
#     #set model to train
#     model.train()
#     # For every batch of data in dataloader i get the data and labels associated
#     # with everything in the batch 
#     for i, batchData in enumerate(dataloader,0):
#         data,label= batchData
#         #plt.imshow(torchvision.utils.make_grid(data))
#         # Use prediction from model  model(data)
#         # Then use its actual label and see its accuracy
#         loss = lossF(model(data),label)
#         loss.backward()
#         # Step one forward optimise
#         optim.step()
#         # Zero any parameter gradients
#         optim.zero_grad()
#         # Every hundred runs print the loss of the NN
#         if(i % 100 == 0):
#             loss = loss.item()
#             print("loss",loss)


# def testNetwork(dataloader, model, loss_fn):
#     model.eval()
...
# # # # # # #   
checkpoint = torch.load('model200Epoch.pth')
